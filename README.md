...
GolfDB 有官方視角標註，是現成的對照實驗（n=1391，量下桿段）：

| 視角 | n | 逐段最短 | 可用比例 |
|---|---|---|---|
| face-on | 454 | **0.76** | **90%** |
| other | 352 | 0.21 | 11% |
| down-the-line | 585 | **0.08** | 1% |

同一個運動、同一份程式，差別只在機位。投手的部分（以骨盆水平位移推論視角）則是
**側面拍最糟**——塌陷落在「前腳著地 → 出手」的 0.73 處，正好是分析的重點；
正／背面拍的塌陷落在 0.09，動作還沒開始，相對無害。

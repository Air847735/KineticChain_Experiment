...
骨架圖看不出球棒在哪裡，所以看不出動作有沒有真的開始。`scripts/visualize.py --crop`
截出同樣兩段的原始影格後，`penn_action/0220` 的前五欄（f0、f1、f4、f4、f4）
可以直接看到**打者還沒開始揮，球棒還在後面**——「前腳著地」「最大分離」「骨盆峰值」
被標在動作尚未發生的位置。

追查訊號證實了這一點。該段的骨盆角速度**真正的峰值在 f22**，不是 f4；規則要求骨盆峰值
早於軀幹峰值（f18），搜尋範圍被限制在 f0–f18，找到的是一個數值只有真峰值三分之一的
雜訊小丘。軀幹同理：真峰值 f27，被壓到 f18。

把 110 段全部算一遍，比較弱標註的位置與**無約束**的訊號峰值位置：

| 事件 | 標在真峰值上 | 偏離中位 | 標註處訊號 / 真峰值（中位） |
|---|---|---|---|
| `pelvis_peak_rotation` | 33% | 8 格 | 0.73 |
| `torso_peak_rotation` | 37% | 2 格 | 0.63 |
| `arm_peak_velocity` | **100%** | 0 格 | 1.00 |

`arm_peak_velocity` 是唯一沒有順序約束的事件，也是唯一 100% 落在真峰值上的事件——
以及唯一 PCE 上得了檯面的事件（0.818）。三件事是同一件事。

這使 `weak_labels.py` 早已記錄的「近端到遠端順序是被建構進去的」有了打擊上的具體數字。
兩個後果：

1. 判斷序列成不成立**必須**繞開弱標註，直接量原始訊號峰值——第 2 節就是這樣做的。
2. 骨盆與軀幹兩個事件的弱標註**已知標錯**，不只是「未驗證」。以它們訓練出來的
   偵測結果不應該當成骨盆／軀幹峰值的量測值使用。
...
| 運動 | 髖線最短（中位） | 塌陷位置（中位） | 可用率 |
|---|---|---|---|
| **`baseball_swing`** | **0.59** | 0.94 | **67%** |
| `tennis_forehand` | 0.55 | 0.43 | 57% |
| `bowling` | 0.51 | 0.87 | 56% |
| `tennis_serve` | 0.42 | 0.74 | 39% |
| `golf_swing` | 0.19 | 0.86 | 33% |
| `baseball_pitch` | 0.19 | 0.17 | 18% |

打者的塌陷位置中位數是 **0.94**（0 = 著地、1 = 觸球）——髖線要到觸球那一刻才轉向鏡頭，
整個揮擊過程都看得見。這是打擊轉播機位的副產品：多數 Penn Action 打擊片段是從投手方向
或側面拍的，打者起始時髖線橫在畫面上，轉到觸球才朝向鏡頭。

投手正好相反：塌陷位置 **0.17**，一進加速期髖線就朝向鏡頭，可用率只有 18%。
原因見 `docs/pitch-analysis.md`。
...
- **旋轉序列**：幾何乾淨的片段有 44.6% 成立，明顯優於隨機。可以看動作是否「用手打」。
  注意這是**直接量原始訊號**得到的，不是模型輸出。
- **不可用**：前腳著地（0.318）與最大分離（0.364）。兩者都定義在姿勢上，
  2D 投影下不穩定。
- **不可用（已知標錯，不只是分數低）**：骨盆峰值與軀幹峰值。弱標註只有 33% / 37%
  落在真正的訊號峰值上，其餘是被順序約束推開的。模型的輸出忠實重現了這個錯誤。

## 未執行

- 沒有真人事件標註。所有分數只代表模型學會弱標註規則的程度；而弱標註的骨盆與軀幹兩個事件
  已由影格對照確認標錯，所以這兩項不是「未驗證」，是**已知有誤**。
- 驗證集僅 22 段。
